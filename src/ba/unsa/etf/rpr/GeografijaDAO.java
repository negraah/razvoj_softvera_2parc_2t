...
public class GeografijaDAO {
    ...
    private GeografijaDAO() {
        ...
        try {
            glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, grad.razvijeni_grad," +
                    "grad.srednje_razvijeni_grad, grad.nerazvijeni_grad FROM grad, drzava WHERE grad.drzava=drzava.id AND drzava.naziv=?");
        } catch (SQLException e) {
            regenerisiBazu();
            try {
                glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, " +
                        "grad.razvijeni_grad, grad.srednje_razvijeni_grad, grad.nerazvijeni_grad FROM grad, drzava WHERE grad.drzava=drzava.id AND drzava.naziv=?");
        ...
        try {
            ...
            promijeniGradUpit = conn.prepareStatement("UPDATE grad SET naziv=?, broj_stanovnika=?, drzava=?, razvijeni_grad=?," +
                    "srednje_razvijeni_grad=?, nerazvijeni_grad=? WHERE id=?");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void dodajGrad(Grad grad) {
        try {
            ...
            dodajGradUpit.setInt(5, grad instanceof RazvijeniGrad ? 1 : 0);
            dodajGradUpit.setInt(6, grad instanceof SrednjeRazvijeniGrad ? 1 : 0);
            dodajGradUpit.setInt(7, grad instanceof NerazvijeniGrad ? 1 : 0);
    ...
    public void izmijeniGrad(Grad grad) {
        try {
            ...
            promijeniGradUpit.setInt(4, grad instanceof RazvijeniGrad ?1 :0);
            promijeniGradUpit.setInt(5,  grad instanceof SrednjeRazvijeniGrad ?1 :0);
            promijeniGradUpit.setInt(6,  grad instanceof NerazvijeniGrad ?1 :0);
            promijeniGradUpit.setInt(7, grad.getId());
